...
func TorrentsFromQBittorrent(torrents []qbittorrent.Torrent) []response.Torrent {
	...
	for _, torrentSrc := range torrents {
		...
		torrentData := response.Torrent{
			...
			EtaMinutes:                  utils.SecondsToMinutes(int64(torrentSrc.ETA)),
			MagnetURI:                   torrentSrc.MagnetURI,
			Leechers:                    int64(torrentSrc.Leechers),
			Seeders:                     int64(torrentSrc.Seeders),
			DateAdded:                   utils.TimeFromUnix(int64(torrentSrc.AddedOn)),
			DateLastActivity:            utils.TimeFromUnix(int64(torrentSrc.LastActivity)),
			DateCompleted:               utils.TimeFromUnix(int64(torrentSrc.CompletionOn)),
			SizeTotalMB:                 utils.BytesToMegabytes(int64(torrentSrc.TotalSize)),
			SizeDownloadedMB:            utils.BytesToMegabytes(int64(torrentSrc.Downloaded)),
			SizeUploadedMB:              utils.BytesToMegabytes(int64(torrentSrc.Uploaded)),
			SizeLeftMB:                  utils.BytesToMegabytes(int64(torrentSrc.AmountLeft)),
			SizeMB:                      utils.BytesToMegabytes(int64(torrentSrc.Size)),
			...
			SpeedDownloadMBps:           utils.BytesPerSecondToMBps(int64(torrentSrc.SpeedDownload)),
			SpeedUploadMBps:             utils.BytesPerSecondToMBps(int64(torrentSrc.SpeedUpload)),
		}
